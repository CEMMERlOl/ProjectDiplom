    ...
    <div id="login">
      <form onsubmit="sendForm(this); return false">
        <fieldset class="clearfix">
          <p><i class="fa-solid fa-user"></i><input type="email" name="email" placeholder="E-mail" required></p>
          <p id="emailp"></p>
      ...
      <p class="text-white">Нет аккаунта? &nbsp;&nbsp;<a href="/Registration.php">Регистрация</a><span class="fontawesome-arrow-right"></span></p>
    </div>
    <!-- Конец Формы -->

    <script>
        async function sendForm(form) {
            let response = await fetch("/Server/Auth.php", {
                method: 'POST',
                body: new FormData(form)
            });
            let result = await response.text();
            if(result == "wremail") {
              emailp.innerText = "Такого пользователя не существует";
              setTimeout(()=>{
                  emailp.innerText = "";
              }, 3000);
            } else if(result == "wrpass") {
              passp.innerText = "Неправильный пароль";
              setTimeout(()=>{
                  passp.innerText = "";
              }, 3000);
